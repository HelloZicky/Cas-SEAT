...
def parse_args():
    ...
    parser.add_argument("--model_name", type=str, default="llavav1.5")
    # Remote model
    parser.add_argument(
        '--model',
        type=str,
...
def main():
    ...
    for item in tqdm(value):
        ...
        system_prompt = "You are a reasoner tasked with solving problems step by step. You need to think about the questions comprehensively and give the question's detailed step by step reasoning. Your reasoning process should be related to the image. Remember that you need to give a final answer in the end. " + query
        system_prompt1 = f"Please provide a detailed reflection on whether there were any mistakes in the previous reasoning process and give a final answer."
        if args.model_name == "llavav1.5":
                conversation = [
        {
        "role": "user",
                ...
                conversation = [
    {
      ...
      "content": [
          ...
          {"type": "image"},
                ...
                prompt = processor.apply_chat_template(conversation, add_generation_prompt=True)
                inputs = processor(images=problem_decoded_image, text=prompt, return_tensors="pt").to("cuda:0")
        ...
        "role": "user",
        "content": [
            {"type": "text", "text": system_prompt + " <|im_end|><|im_start|>assistant " + response + "<|im_end|><|im_start|>user " + system_prompt1},
            {"type": "image",
             "image": image_path},
            ],
        }
