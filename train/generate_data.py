...
system_prompt = f'''**You are a Reasoner tasked with solving problems step by step.You need give the problem's step by step solution.Each step needs to include objective information observed from the image and the corresponding reasoning process. Follow these instructions:**
1. **Self-Evaluation:**
...
2. **Final Result:**
   - Once the solution is complete, present the final result in this format:
     ```
     [FINAL Result]: \boxes [Final Result]
     ```
...
def main():
    ...
    for i, problem_id in enumerate(tqdm(test_pids)):
        ...
        if args.dataset_name == "Math360k":
            query = problem['conversations'][0]["value"]
            image_path =  problem['image']
            problem_decoded_image = Image.open(image_path)
            query = query.replace("<image>", "") 
            answer = problem['conversations'][1]["value"]
            #system_prompt = '''**You are a Reasoner tasked with solving problems step by step.Each step needs to include objective information observed from the image and the corresponding reasoning process.**'''
...
Ensure the reasoning process is accurate, and invoke self-evaluation whenever necessary.\n'''
        ...
        "content": [
            ...
            {"type": "image"},
                ...
                inputs = processor(images=problem_decoded_image, text=prompt, return_tensors='pt')
